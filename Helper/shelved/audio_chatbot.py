...
class Recorder:
    ...
    def chatGroq(user_query):
        ...
        if askText:
            ...
            system = "You are a helpful assistant. You mostly give one line answers to human queries else keep it short and sarcastic. You do not apologise"
            human = "{query}"
        ...
        for idx, chat_message in enumerate(st.session_state.chat_history):
            if chat_message["role"] == "human":
                ...
                if idx == len(st.session_state.chat_history) - 1 and askText:
                    ...
                    Recorder.typewriter_effect(f"AI: {chat_message['content']}", placeholder)
                else:
